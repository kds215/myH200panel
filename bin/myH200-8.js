...
var isJSfilename = "H200-8.js";
var isJSfileURL = "http://localhost/Honeywell/bin/" + isJSfilename ;
...
var powerON = true;
var stopON = true;
var isRunMode = false;  // as in Test Memory Run

/* H200 with 32K of core memory and 16 registers */

// Global Content | Address | Register (decimal) values
var ContValue = 0;
var AddrValue = 0;
var RegValue  = 0;
...
function pDispAddrContent( imgId, newSrc ){

    if ( isDebug )
        console.log('pDispAddrContent()');

    if (( ! powerON ) || ( isRunMode )) {  // do nothing on PowerOff or when in Run mode
        return;
    }
...
function pEnterAddrContent( imgId, newSrc ){
...
function pDispRegAddrContent( imgId, newSrc ){
...
function pDispAddrPlus1( imgId, newSrc ){
...
function pDispAddrMinus1( imgId, newSrc ){
...
function pEnterRegAddr( imgId, newSrc ){
...
function pInit( butId, newSrc ){

    if (( ! powerON ) || ( isRunMode )) {  // do nothing on PowerOff or when in Run mode
        return;
    }

    putImage(  butId, newSrc ); // light-up INIT button
...
function runTestMemory() { // on sense switch 1
...
// simulate H200 panel button flashing with different (timed) classes
...
function runClearMemory() { // on sense switch 2
};
